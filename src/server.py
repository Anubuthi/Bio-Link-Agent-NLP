...
if __name__ == "_main_":
    ...
    # --- FIX START: Rely on env variables or defaults, remove non-existent args ---
    # We still use the environment variables to guide the user on the expected URL,
    # but we remove the arguments that FastMCP doesn't accept.
    
    SERVER_HOST = os.getenv("SERVER_HOST", "127.0.0.1")
    SERVER_PORT = int(os.getenv("SERVER_PORT", 8000))

    # CRITICAL FIX: The TypeError means FastMCP.run() DOES NOT accept 'host'/'port'.
    # It must rely on Uvicorn's defaults or environment variables.
    sys.stderr.write(f"INFO: Starting FastMCP server. Check Uvicorn output for URL (likely http://{SERVER_HOST}:{SERVER_PORT})\n")
    
    # Run the server without the arguments that caused the TypeError
    mcp.run()
    # --- FIX END ---
...
def search_medical_data(query: str) -> str:
    ...
    Args:
        query: Medical research query string
    
    Returns:
        Formatted string with research papers and active trials
    """
    try:
        sys.stderr.write(f"DEBUG: search_medical_data query={query}\n")
        papers = pubmed.fetch_research(query, max_results=3)
...
def search_pubmed(query: str, max_results: int = 5) -> str:
    ...
    Args:
        query: The FULL original user question or a cleaned version
        max_results: Maximum number of papers to return (default: 5)
    
    Returns:
        JSON string containing paper metadata
    """
    try:
        sys.stderr.write(f"DEBUG: search_pubmed query={query}, max_results={max_results}\n")
        papers = pubmed.fetch_research(query, max_results=max_results)
...
def search_trials(
    ...
    Args:
        condition: Disease/condition name (e.g., "lung cancer")
        limit: Maximum number of trials to return (default: 5)
        country: Optional country filter (e.g., "United States")
    
    Returns:
        JSON string containing active trial information
    """
    try:
        sys.stderr.write(
            f"DEBUG: search_trials condition={condition}, limit={limit}, country={country}\n"
...
def match_trials_semantic(
    ...
    Uses vector similarity to match patient clinical notes with trial eligibility criteria.
    Applies age, sex, and location filters before semantic matching.

    Args:
        condition: Primary medical condition (e.g., "lung cancer")
        patient_note: Free-text clinical note describing symptoms, prior treatments, comorbidities, etc.
        age: Patient age in years (optional)
        sex: Patient sex - "male", "female", or "all" (optional)
        country: Patient country for location filtering (e.g., "United States")
        limit: Maximum number of matched trials to return (default: 5)

    Returns:
        Formatted string listing top-matched trials with eligibility details and match scores
    """
    try:
        sys.stderr.write(
            f"DEBUG: match_trials_semantic cond={condition}, age={age}, sex={sex}, country={country}, limit={limit}\n"
...
def build_knowledge_graph(
    ...
    Fetches papers from PubMed and optionally trials from ClinicalTrials.gov,
    then ingests them into Neo4j to create a queryable knowledge graph.
    
    Args:
        topic: Research topic or medical condition
        max_papers: Maximum number of PubMed papers to fetch (default: 10)
        max_trials: Maximum number of clinical trials to fetch (default: 10)
        include_trials: Whether to include clinical trials (default: True)
    
    Returns:
        JSON summary of ingested papers and trials
    """
    try:
        sys.stderr.write(
            "DEBUG: build_knowledge_graph "
...
if __name__ == "__main__":
    ...
    sys.stderr.write("="*60 + "\n")
    sys.stderr.write("🚀 Starting Bio-Link-Agent MCP Server (SSE Transport)\n")
    sys.stderr.write("="*60 + "\n")
    sys.stderr.write(f"📍 Server will be available at: http://{SERVER_HOST}:{SERVER_PORT}\n")
    sys.stderr.write(f"📍 SSE endpoint: http://{SERVER_HOST}:{SERVER_PORT}/sse\n")
    sys.stderr.write(f"📍 Health check: http://{SERVER_HOST}:{SERVER_PORT}/health\n")
    sys.stderr.write("="*60 + "\n\n")
    
    # Run with SSE transport (HTTP-based)
    # This creates an HTTP server with Server-Sent Events for MCP communication
    mcp.run(transport="sse")
